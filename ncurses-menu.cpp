...
int print_menu (int sty, int x, int alts, int width,
            std::string title, std::vector<std::string> entries, int start)
{
    ...
    if ((int)title.size() + 2 > width)    /* "width" cannot be less than */
        width = (int)title.size() + 2;    /* the width of the strings    */
    ...
    if ((width - (int)title.size()) % 2 != 0) /* If it's not possible to */
    {                                       /* perfectly center the    */
        blankspace2 = (width - (int)title.size() + 1) / 2;
        blankspace1 = blankspace2 - 1;      /* menu title, it will be  */
    }                                       /* placed slightly left.   */

    else
    {
        blankspace1 = (width - (int)title.size()) / 2;
