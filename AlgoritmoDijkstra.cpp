...
int AlgoritmoDijkstra::calcular_ruta()
{
	int sum = 0;
...
std::vector<std::pair<int,int>> AlgoritmoDijkstra::obtener_aristas(std::pair<int,int> a)
{
...
void AlgoritmoDijkstra::iniciar_busqueda()
{
	...
	pq.push({0,{x_lab(),y_lab()}});
	
	while(!pq.empty()) // mientras la cola no esta vacia
	{
		int dNodo = pq.top().first; // distancia al nodo
		std::pair<int,int> a = pq.top().second; // coordenadas punto actual
		pq.pop();
		...
		for(auto par : obtener_aristas(a))
		{
			...
			if(laberinto[y_lab()][x_lab()] != muro && mapa_vistos[y_lab()][x_lab()] == 0)
			{
				...
				int dist = dNodo + (laberinto[y_lab()][x_lab()] - '0') * -1;
				pq.push({dist,{x_lab(),y_lab()}});
				mapa_vistos[y_lab()][x_lab()] = 1;
				
				if(y == fy && x == fx)
				{
					std::priority_queue<std::pair<int, std::pair<int,int>>> vacia;
					std::swap(pq, vacia);
				}
...
void AlgoritmoDijkstra::calculo_final()
{
